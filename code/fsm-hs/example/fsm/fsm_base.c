...
fsm_err_t fsm_event_init(fsm_handler_base_t* h, uint8_t* buffer ,uint16_t size)
{
    ...
    name[3] = temp[0];
    name[4] = temp[1];
    h->eq = rt_mb_create((char *)name, (rt_size_t)size, RT_IPC_FLAG_PRIO);
	if(h->eq == NULL)
	{
		return FSM_ERROR;
	}
...
{
	h->eq = OSQCreate((void**) buffer, size);
	if(h->eq == NULL)
	{
		return FSM_ERROR;
	}
